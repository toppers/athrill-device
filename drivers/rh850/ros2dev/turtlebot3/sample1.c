...
//START=================>
static RosDevScanDataType scan_data;
static RosDevTwistType cmd_vel;
...
static rosdev_bool do_foward(void) {
  rosdev_bool is_stop = FALSE;
  cmd_vel.linear.x = 0;
...
static rosdev_bool turn_right(void) {
	rosdev_bool is_stop = FALSE;
  cmd_vel.angular.z = 0;
...
void turtlebot3_controller(void)
{

	syslog(LOG_NOTICE, "TurtleBot3 Controller:start");
	...
	req_pub.id = 1;
	req_pub.ret = 0;
	req_pub.datalen = sizeof(cmd_vel);
	req_pub.ptr = (RosDevDataPtrType)&cmd_vel;

	req_sub.id = 1;
	req_sub.ret = 0;
	req_sub.datalen = sizeof(scan_data);
	req_sub.ptr = (RosDevDataPtrType)&scan_data;
	...
	while (1) {
		...
		if (mode == RoboMode_INIT) {
			float d = sarch_all();
			if (d > 0.0f && d <= 0.08f) {
				syslog(LOG_NOTICE, "d=%f MOVE\n", d);
				mode = RoboMode_RUN;
			}
			else {
				syslog(LOG_NOTICE, "WATING d=%f\n", d);
			}
		}
		else {
			do_control();
